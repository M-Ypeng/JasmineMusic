<template>
  <div class="listenNowPage">
    <div class="listenNowTitle">
      <div class="title">现在就听</div>
      <svg
        t="1664978442122"
        class="icon"
        viewBox="0 0 1024 1024"
        version="1.1"
        xmlns="http://www.w3.org/2000/svg"
        p-id="11537"
        width="30"
        height="30"
      >
        <path
          d="M983.8 312.7C958 251.7 921 197 874 150c-47-47-101.8-83.9-162.7-109.7C648.2 13.5 581.1 0 512 0S375.8 13.5 312.7 40.2C251.7 66 197 102.9 150 150c-47 47-83.9 101.8-109.7 162.7C13.5 375.8 0 442.9 0 512s13.5 136.2 40.2 199.3C66 772.3 102.9 827 150 874c47 47 101.8 83.9 162.7 109.7 63.1 26.7 130.2 40.2 199.3 40.2s136.2-13.5 199.3-40.2C772.3 958 827 921 874 874c47-47 83.9-101.8 109.7-162.7 26.7-63.1 40.2-130.2 40.2-199.3s-13.4-136.2-40.1-199.3z m-55.3 375.2c-22.8 53.8-55.4 102.2-96.9 143.7s-89.9 74.1-143.7 96.9C632.2 952.1 573 964 512 964s-120.2-11.9-175.9-35.5c-53.8-22.8-102.2-55.4-143.7-96.9s-74.1-89.9-96.9-143.7C71.9 632.2 60 573 60 512s11.9-120.2 35.5-175.9c22.8-53.8 55.4-102.2 96.9-143.7s89.9-74.1 143.7-96.9C391.8 71.9 451 60 512 60s120.2 11.9 175.9 35.5c53.8 22.8 102.2 55.4 143.7 96.9s74.1 89.9 96.9 143.7C952.1 391.8 964 451 964 512s-11.9 120.2-35.5 175.9z"
          fill="#e63e43"
          p-id="11538"
        ></path>
        <path
          d="M693.2 658.2c-20.6-20.6-44.2-37.3-69.7-49.7 51.7-35.6 85.7-95.2 85.7-162.6 0-108.8-88.5-197.2-197.2-197.2s-197.2 88.5-197.2 197.2c0 67.4 34 127 85.7 162.6-25.5 12.4-49.1 29-69.7 49.7-22 22-39.5 47.2-52 74.7-9.1 19.9 5.5 42.5 27.4 42.5h0.3c11.7 0 22.3-6.9 27.2-17.6 31-67.5 99.3-114.6 178.4-114.6 79.1 0 147.4 47 178.4 114.6 4.9 10.7 15.4 17.6 27.2 17.6h0.3c21.8 0 36.4-22.6 27.4-42.5-12.7-27.5-30.2-52.8-52.2-74.7zM512 583.1c-75.6 0-137.2-61.5-137.2-137.2S436.4 308.8 512 308.8 649.2 370.3 649.2 446 587.6 583.1 512 583.1z"
          fill="#e63e43"
          p-id="11539"
        ></path>
      </svg>
    </div>
    <div class="recentPlays">
      <div class="recentPlaysTitle">歌曲推荐</div>
      <div class="recentPlaysBox">
        <div
          class="recentPlaysItem"
          v-for="item in personalizedList"
          :key="item.id"
          @click="detailsBtn(item)"
        >
          <img
            style="width: 100%; border-radius: 5px"
            :src="item.picUrl"
            alt=""
          />
          <div class="desc">{{ item.name }}</div>
        </div>
      </div>
    </div>
    <div class="author">
      <van-image
        round
        width="40"
        height="40"
        src="http://p2.music.126.net/BbR3TuhPULMLDV0MjczI4g==/109951165793869641.jpg?param=40y40"
      />
      <div class="works">
        <div style="color: #9f9f9f">President Chou</div>
        <div style="font-size: 18px; font-weight: bold">周杰伦</div>
      </div>
    </div>
    <div class="containt">
      <div class="bannerBox">
        <div @click="playSong(item.id)" v-for="item in songList" :key="item.id">
          <img
            style="width: 100%; border-radius: 5px"
            :src="item.al.picUrl + '?param=300y300'"
            alt=""
          />
        </div>
      </div>
    </div>
  </div>
</template>

<script>
import { personalized, artistSongs } from "../api/index";

export default {
  components: {},
  data() {
    return {
      songList: [],
      personalizedList: [],
    };
  },
  created() {
    this.search();
    this.personalized();
  },
  methods: {
    detailsBtn(item) {
      this.$router.push({
        name: "listSongs",
        query: {
          id: item.id,
        },
      });
    },

    playSong(id) {
      this.$emit("getSong", id);
    },
    async search() {
      let res = await artistSongs();
      if (res.code == "200") {
        this.songList = res.songs.splice(1, 8);
      }
    },

    async personalized() {
      let res = await personalized();
      if (res.code == "200") {
        this.personalizedList = res.result;
      }
    },
  },
};
</script>

<style lang="scss" scoped>
.listenNowPage {
  padding: 10px;
  margin-bottom: 100px;
}
.listenNowTitle {
  display: flex;
  justify-content: space-between;
  align-items: center;
  height: 50px;
}
.title {
  font-size: 28px;
  font-weight: bold;
}
.recentPlaysTitle {
  font-size: 20px;
  padding: 10px 0;
}
.recentPlays {
  margin-bottom: 10px;
}
.recentPlaysBox {
  display: flex;
  min-height: 200px;
}
.recentPlaysItem {
  overflow: hidden;
  width: 50%;
  padding: 5px;
}
.desc {
  width: 100%;
  font-size: 16px;
  overflow: hidden;
  text-overflow: ellipsis;
  white-space: nowrap;
}
.containt {
  width: 100%;
  min-height: 200px;
  position: relative;
  overflow-y: scroll;
}
.bannerBox {
  position: absolute;
  left: 0px;
  width: 400%;
  display: flex;
  div {
    width: 50%;
    padding: 5px;
  }
}
.author {
  padding: 0 5px;
  display: flex;
  align-items: center;
}
.works {
  padding-left: 5px;
}
</style>